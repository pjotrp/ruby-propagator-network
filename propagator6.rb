...
#
# Continuing with the 0MQ example we add some propagator
# functionality.  The first one is to create a propagator that allows
# running multiple propagators where the first one wins as a
# input. Note the computation is no longer deterministic as we can
# have two outcomes (see p_plus_alt). Still, this allows alternative
# computation routes to run at the same time - say a CPU version and a
# GPU version.  The second propagator takes two of these inputs and
# compares them (as a test). This is to encourage determinism. We make
# these tests optional.
#
